 ...
 * @retval         none
 */
void cascade_PID_init(cascade_pid_t *c_pid, const float oPID[3], const float iPID[3], float oPID_max_out, float oPID_max_iout, float iPID_max_out, float iPID_max_iout)
{
 ...
 */
float cascade_PID_calc(cascade_pid_t *c_pid, float o_ref, float i_ref, float o_set)
{
  c_pid->s_set = o_set;
  c_pid->s_fdb = o_ref;
  c_pid->v_fdb = i_ref;
